...
console.log('📦 stock-app.js is loading...');
console.log('Global window scope check at load time:');
console.log('  - window.stok:', window.stok);
console.log('  - window.kategoriList:', window.kategoriList);
console.log('  - window.upbjjList:', window.upbjjList);
...
createApp({
  ...
  data() {
    return {
      ...
      // Inline validation errors for add/edit forms
      addErrors: {},
      editErrors: {},
  ...
  methods: {
    ...
    addItem() {
      ...
      // Validasi field required (inline)
      if (!this.newItem.kode || !this.newItem.kode.toString().trim()) {
        this.addErrors.kode = 'Kode harus diisi';
      }
      if (!this.newItem.judul || !this.newItem.judul.toString().trim()) {
        this.addErrors.judul = 'Judul harus diisi';
      }

      // Cek duplikat kode
      if (this.newItem.kode && this.items.some(i => i.kode === this.newItem.kode)) {
        this.addErrors.kode = 'Kode sudah digunakan';
      }
    ...
    updateItem() {
      ...
      // Validasi field required (inline)
      if (!this.currentItem || !this.currentItem.kode || !this.currentItem.kode.toString().trim()) {
        this.editErrors.kode = 'Kode harus diisi';
      }
      if (!this.currentItem || !this.currentItem.judul || !this.currentItem.judul.toString().trim()) {
        this.editErrors.judul = 'Judul harus diisi';
      }

      // Cek duplikat kode (exclude current item by comparing index)
      const idxExisting = this.items.findIndex(i => i.kode === this.currentItem.kode);
      const duplicate = this.items.find((i, idx) => i.kode === this.currentItem.kode && idx !== idxExisting);
      if (duplicate) {
        this.editErrors.kode = 'Kode sudah digunakan oleh item lain';
      }
    ...
    initializeData() {
      try {
        ...
        console.debug('[stock] initializeData() (methods) globals:', {
          window_stok: window.stok,
          window_kategoriList: window.kategoriList,
          window_upbjjList: window.upbjjList
        });

        // Try to get stok from window object
        let stokData = window.stok || (typeof stok !== 'undefined' ? stok : []);
        const kategoriData = window.kategoriList || (typeof kategoriList !== 'undefined' ? kategoriList : []);
        const upbjjData = window.upbjjList || (typeof upbjjList !== 'undefined' ? upbjjList : []);

        // If stokData empty, use embedded fallback so UI renders immediately
        ...
        if (stokData && stokData.length > 0) {
          this.items = JSON.parse(JSON.stringify(stokData));
        } else {
          this.items = [];
        }
        ...
        if (kategoriData && kategoriData.length > 0) {
          this.kategoriList = kategoriData;
        }

        // Load upbjj
        if (upbjjData && upbjjData.length > 0) {
          this.upbjjList = upbjjData;
        }

        console.debug('[stock] initializeData() (methods) done, items length:', this.items.length, 'kategoriList length:', this.kategoriList.length, 'upbjjList length:', this.upbjjList.length);
      } catch (err) {
        console.error('[stock] initializeData() (methods) error:', err);
      }
    }
  },
  ...
  //
  mounted() {
    // Initialize now (if data already present) and also re-run when dataLoaded fires
    try {
      console.debug('[stock] mounted() start');
      this.initializeData();
      // use bound handler so `this` is correct when called from event
      const handler = this.initializeData.bind(this);
      this._dataLoadedHandler = handler;
      window.addEventListener('dataLoaded', handler);

      // Global keydown handler: close modals on Escape
      ...
      console.debug('[stock] mounted() finished, listener attached');
    } catch (err) {
      console.error('[stock] mounted() error:', err);
    }
  },
  beforeUnmount() {
    // Clean up global listeners when app unmounts
    try {
      if (this._dataLoadedHandler) window.removeEventListener('dataLoaded', this._dataLoadedHandler);
      if (this._globalKeydown) window.removeEventListener('keydown', this._globalKeydown);
    } catch (e) {
      console.warn('Error removing listeners on beforeUnmount', e);
    }
  },
}).mount('#stockApp');
